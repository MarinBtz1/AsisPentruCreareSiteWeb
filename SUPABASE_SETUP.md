...
## Pași pentru conectarea bazei de date Supabase

### 1. Instalare completă
Pachetul `@supabase/supabase-js` a fost deja instalat în proiect.

### 2. Crearea tabelului în Supabase

Accesează dashboard-ul Supabase: https://supabase.com/dashboard/project/rptjcilhsoparwfrwcxl

#### Opțiunea 1: Folosind SQL Editor
1. Mergi la **SQL Editor** în panoul stâng
2. Creează o nouă query
3. Rulează următorul script SQL:
...
-- Inserează datele inițiale din items.json
...
#### Opțiunea 2: Folosind Table Editor
1. Mergi la **Table Editor** în panoul stâng
2. Click pe **New Table**
3. Numele tabelului: `items`
4. Adaugă următoarele coloane:
   - `id` (text, primary key)
   - `title` (text, not null)
...
5. După crearea tabelului, activează RLS:
   - Mergi la **Authentication** > **Policies**
   - Creează o politică pentru SELECT care permite acces public

### 3. Configurarea Row Level Security (RLS)

După crearea tabelului, asigură-te că:
1. RLS este activat pentru tabelul `items`
2. Există o politică care permite citirea publică (SELECT) pentru toți utilizatorii

### 4. Testarea conexiunii

După configurarea tabelului:
1. Rulează aplicația: `npm start`
2. Verifică în consola browserului dacă există erori
3. Datele ar trebui să se încarce din Supabase în loc de fișierul JSON

### 5. Structura datelor

Tabelul `items` trebuie să aibă următoarea structură:
- `id`: TEXT (primary key)
- `title`: TEXT (not null)
- `category`: TEXT (nullable)
- `price`: NUMERIC (not null)
- `image`: TEXT (not null)
- `description`: TEXT (nullable)
- `availableSizes`: JSONB (nullable) - array de string-uri
- `stock`: INTEGER (default 0)

### Note importante

- Credențialele Supabase sunt configurate în `src/environments/environment.ts` și `src/environments/environment.prod.ts`
- Serviciul `SupabaseService` gestionează conexiunea la Supabase
